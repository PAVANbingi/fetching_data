...
=======================================
##GOOGLE SHEETS :
Name	Email	Phone 	ProjectDetails	How_did_you_hear_aboutUs	Subscribe to Newsletter	Timestamp
# APP SCRIPT :

const sheets = SpreadsheetApp.openByUrl("https://docs.google.com/spreadsheets/d/1YshqlxctqbJ5THuNGFDUkH3XMLPZAB2frENYE72X2uM/edit?gid=0");
// Replace "Page1" with your actual sheet name if necessary
const sheet = sheets.getSheetByName("Page1");

function doPost(e) {
  let data = e.parameter;

  // Append a new row with all the required columns
  sheet.appendRow([
    data.Name,                                  // Name
    data.Email,                                 // Email
    data.Phone || "N/A",                        // Phone (optional, default "N/A" if not provided)
    data.ProjectDetails,                        // Project Details (ensure the field name is "ProjectDetails")
    data.How_did_you_hear_aboutUs,              // How did you hear about us (ensure the field name is "How_did_you_hear_aboutUs")
   data.subscribe === "on" ? "Yes" : "No",     // Subscribe to Newsletter (Yes/No based on checkbox)
